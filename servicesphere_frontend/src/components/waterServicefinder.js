...
const WaterServiceUsers = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('http://localhost:3000/api/v1/users/water-service');
        if (!response.ok) {
  ...
  return (
    <div>
      ...
      <h3 className='finder'>Water Services</h3>
      {/* Search bar */}
      <input
        type="text"
...
export default WaterServiceUsers;
